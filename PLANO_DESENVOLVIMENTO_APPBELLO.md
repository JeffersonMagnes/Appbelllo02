...
| AUTH-001 | **Implementação Web concluída; ativação server-side bloqueada pelo deploy atual** | login de funcionário deixou de consultar e-mail/PIN no navegador; sessão HttpOnly assinada, expiração, logout, rate limit e APIs tenant-scoped implementados; 10 contratos, typecheck, lint e build aprovados. Netlify CLI + runtime v4 publica páginas mas não Route Handlers (`/api/*` retorna 404); tentativa v5 foi revertida após incompatibilidade do deploy manual, sem deixar 502 em produção |
