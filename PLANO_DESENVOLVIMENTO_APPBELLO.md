...
| WEB-DOMAIN | **Configurado; HTTPS aguardando propagação DNS** | `appbello.com.br` definido como principal e `www` como alias no Netlify; registros A/CNAME cadastrados no Registro.br; URLs internas migradas para o domínio próprio. Apex já alcança o Netlify, mas o certificado e o `www` ainda aguardam propagação |
